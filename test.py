...
model_type = 1 # 0: torch.nn.LSTM(); 1: MyLSTM()
...
class TextLSTM(nn.Module):
    def __init__(self):
        ...
        self.LSTM = MyLSTM(emb_size, n_hidden)
...
def train_LSTMlm():
    ...
    for epoch in range(all_epoch):
        ...
        if (epoch + 1) % save_checkpoint_epoch == 0:
            torch.save(model, f'models/MyLSTM_model_epoch{epoch + 1}.bin')
...
if __name__ == '__main__':
    ...
    print("\nTest the LSTMLM……………………")
    select_model_path = "models/LSTMlm_model_epoch5.bin"
    test_LSTMlm(select_model_path)

    print("\nTest the LSTMLM……………………")
    select_model_path = "models/MyLSTM_model_epoch5.bin"
    test_LSTMlm(select_model_path)